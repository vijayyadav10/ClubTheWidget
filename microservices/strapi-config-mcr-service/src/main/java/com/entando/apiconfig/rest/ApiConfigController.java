 ...
 */
@RestController
// @RequestMapping({"#{environment.SERVER_SERVLET_CONTEXT_PATH}", "/api/v1"})
// @RequestMapping({"/api/config"})
// @RequestMapping({"#{environment.SERVER_SERVLET_CONTEXT_PATH + '/api'}", "${customIngress}/api"})
@RequestMapping({"/api", "${customIngress}/api"})
// @RequestMapping({"/api/config", "#{systemProperties['server.servlet.context-path'] + '/api'}"})
public class ApiConfigController {

	private final Logger logger = LoggerFactory.getLogger(ApiConfigController.class);

	@Value("${customIngress}/api")
	private String bar;

	@Value("#{environment.SERVER_SERVLET_CONTEXT_PATH + '/api'}")
	private String tar;
